...
class Section:
    ...
    def setupSubjects(self):
        ...
        model.setHorizontalHeaderLabels(
            ["ID", "Available", "Shared", "Subject Code", "Subject Name", "Share ID"]
        )
        tree.setModel(model)
        ...
        for subject in subjects:
            ...
            for sharing in sharedSubjects:
                ...
                shared.setCheckState(QtCore.Qt.CheckState.Checked)
    ...
    def toggleSharing(self, item):
        if item.column() == 2:
            ...
            if (
                shareToggle == QtCore.Qt.CheckState.Checked
                and not self.model.item(item.row(), 2).text()
            ):
                shareData = Share.Share(subjectId, self.id).getShareData()
                if not shareData[0]:
                    self.model.item(item.row(), 2).setCheckState(
                ...
                self.model.item(item.row(), 1).setCheckState(
                    QtCore.Qt.CheckState.Checked
                )
                ...
                if int(self.model.item(item.row(), 5).text()) in self.shareId:
                    ...
                    self.removeShareId.append(
                        int(self.model.item(item.row(), 5).text())
                    )
                self.model.item(item.row(), 5).setText("")
                self.model.item(item.row(), 2).setText("")
        elif item.column() == 1:
            if (
                self.model.item(item.row(), 1).checkState()
                == QtCore.Qt.CheckState.Unchecked
                and self.model.item(item.row(), 5).text()
            ):
                self.model.item(item.row(), 2).setCheckState(
                    QtCore.Qt.CheckState.Unchecked
                )
    ...
    def fillForm(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT name, schedule, stay FROM sections WHERE id = ?", [self.id]
        )
...
class Tree:
    def __init__(self, tree):
        ...
        self.model = model = QtGui.QStandardItemModel()
        model.setHorizontalHeaderLabels(
            ["ID", "Available", "Name", "Stay in Room", "Operation"]
        )
        tree.setModel(model)
        tree.setColumnHidden(0, True)
