...
class ResultViewer:
    ...
    def parseResultDetails(self):
        ...
        self.parent.lblCPU.setText(
            "Average CPU Usage: {}%".format(
                round(result["resource"]["cpu"]),
            )
        )
        self.parent.lblMemory.setText(
            "Average Mem Usage: {} MB".format(
                round(result["resource"]["memory"]),
            )
        )
        ...
        if category == 0:
            ...
            for subject, details in subjects.items():
                if not len(details):
                    continue
                instructor = (
                    "" if not details[1] else rawData["instructors"][details[1]][0]
                )
                data.append(
                    {
                        "color": None,
                        ...
                        "instances": [
                            [day, details[3], details[3] + details[4]]
        ...
        # Room
            ...
            for section, details in sections.items():
                for subject, subjectDetail in details["details"].items():
                    ...
                    instructor = (
                        ""
                        if not subjectDetail[1]
                        else rawData["instructors"][subjectDetail[1]][0]
                    )
                    data.append(
                        {
                            "color": None,
                            ...
                            "instances": [
                                [
                                    day,
                                    subjectDetail[3],
                                    subjectDetail[3] + subjectDetail[4],
                                ]
                                for day in subjectDetail[2]
                            ],
                        }
                    )
        # Instructor
    ...
    def export(self):
        directory = QtWidgets.QFileDialog().getExistingDirectory(
            None, "Select Directory for Export"
        )
        if not directory:
            return False
        ...
        with open(
            "{}/sections_schedule.csv".format(directory), "w", newline=""
        ) as file:
            ...
            for section, subjects in chromosome["sections"].items():
                ...
                schedule = [
                    ["" for j in range(6)]
                    for i in range(
                        self.settings["ending_time"]
                        - self.settings["starting_time"]
                        + 1
                    )
                ]
                for subject, details in subjects["details"].items():
                    ...
                    for timeslot in range(details[3], details[3] + details[4]):
                        for day in details[2]:
                            schedule[timeslot][day] = "{} - {} - {}".format(
                                rawData["subjects"][subject][2],
                                rawData["rooms"][details[0]][0],
                                instructor,
                            )
                for timeslot in range(
                    self.settings["starting_time"], self.settings["ending_time"] + 1
                ):
                    writer.writerow(
                        [
                            timeslots[timeslot],
                            *schedule[timeslot - self.settings["starting_time"]],
                        ]
                    )
                writer.writerow([""])
        # Create schedule for instructors
        with open(
            "{}/instructors_schedule.csv".format(directory), "w", newline=""
        ) as file:
            writer = csv.writer(file, dialect="excel")
                ...
                for section, subjects in chromosome["sections"].items():
                    for subject, details in subjects["details"].items():
                        ...
                        instructor = (
                            ""
                            if not details[1]
                            else rawData["instructors"][details[1]][0]
                        )
                        for timeslot in range(details[3], details[3] + details[4]):
                            for day in details[2]:
                                schedule[timeslot][day] = "{} - {} - {}".format(
