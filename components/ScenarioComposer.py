...
class ScenarioComposer:
    ...
    def getInstructors(self):
        self.cursor.execute(
            "SELECT id, name, hours, schedule FROM instructors WHERE active = 1"
        )
    ...
    def getRooms(self):
        self.cursor.execute(
            "SELECT id, name, type, schedule FROM rooms WHERE active = 1"
        )
    ...
    def getSubjects(self):
        self.cursor.execute(
            "SELECT id, name,id_to_name , hours, code, instructors, type FROM subjects"
        )
    ...
    def getSections(self):
        self.cursor.execute(
            "SELECT id, name, schedule, subjects FROM sections WHERE active = 1"
        )
